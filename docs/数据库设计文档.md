...
> 复用平台原有的用户、智能体、工作流、知识库等表结构，  
> 仅新增与"健康+健身"业务相关的表，并对部分核心表做适当扩展。
...
1. **复用平台核心能力**  
   - 不重复造轮子，直接使用平台已有表结构：
     ...
     - 工作流相关：`workflow_instance`, `workflow_log`
     - 知识库相关：`kb_category`, `kb_article`
...
| 知识库 | 2 | `kb_category`, `kb_article` | 复用平台 |
| 会话管理 | 2 | `conversation`, `message` | 复用平台（扩展字段） |
| 工作流基础 | 2 | `workflow_instance`, `workflow_log` | 复用平台 |
| 健康档案 | 1 | `health_profile` | FitPulse 新增 |
| 健康数据记录 | 4 | `weight_log`, `workout_log`, `sleep_log`, `diet_log` | FitPulse 新增 |
| 饮食拍照识别 | 2 | `diet_photo`, `diet_food_item` | FitPulse 新增 |
| 健康业务 | 3 | `health_plan`, `weekly_report`, `risk_alert` | FitPulse 新增 |
| **健康档案** | 1张 | health_profile | FitPulse 新增 |
| **健康数据记录** | 4张 | weight_log, workout_log, sleep_log, diet_log | FitPulse 新增 |
| **健康业务** | 3张 | health_plan, weekly_report, risk_alert | FitPulse 新增 |
...
2. 触发工作流 → `workflow_instance` 记录执行
3. 工作流读取：`health_profile`, 最近的 `weight_log`, `workout_log`
4. 工作流调用智能体（`agent`）+ RAG，生成训练/生活方式计划
5. 将结果写入 `health_plan`，前端通过 `/api/health/plans/current` 展示
...
2. 调用智能体生成周度报告 → 写入 `weekly_report`
3. 用户登录后提示「有新的周报」
...
触发后：
1. 创建 `workflow_instance`（类型 risk_alert）
2. 可调用智能体生成提醒文案 → 写入 `risk_alert`
3. 前端通过 `/api/health/alerts` 提示用户查看

#### 2.2.4 一日三餐拍照识别 & 红黄绿灯评估
...
- **用途**：存储智能体的 Prompt、工具权限、知识库范围、默认 LLM 模型
...
- `PUT /api/health/profile` - 更新健康档案（触发初次评估工作流）
...
**说明**：以「一顿饭」为粒度；兼容手动记录和拍照识别；  
支持总卡路里与整体红黄绿灯评估（兼顾减脂、慢病饮食要求）。
...
#### 4.5.2 饮食图片表 diet_photo

**说明**：存储与某一顿饭关联的图片及识别状态/结果；支持一餐多图。
...
#### 4.5.3 饮食明细表 diet_food_item

**说明**：以「单个食物」为粒度，存储食物名称、分量、卡路里、单品红黄绿灯及慢病相关风险标签。
...
- `POST /api/health/diets` - 手动记录饮食
- `POST /api/health/diets/photo` - 拍照上传并触发 AI 识别和红黄绿灯评估
- `GET /api/health/diets/{id}` - 获取某一餐的详细信息（包含图片和食物明细）
...
### 4.8 健康风险提醒表 (risk_alert)

#### 4.8.1 表信息

**表名**: `risk_alert`  
**说明**: 存储规则/工作流触发的健康或训练风险提醒
